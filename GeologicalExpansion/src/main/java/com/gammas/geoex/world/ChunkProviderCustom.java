package com.gammas.geoex.world;
...
public class ChunkProviderCustom implements IChunkProvider{
	...
	public ChunkProviderCustom(WorldServer worldObjIn, IChunkLoader chunkLoaderIn, IChunkGenerator chunkGeneratorIn) {
	 ...
	 * 
	 * Queueing a chunk for unload does <b>not</b> guarantee that it will be
	 * unloaded, as any request for the chunk will unqueue the chunk.
	 */
			...
			if (chunk != null || !(this.chunkLoader instanceof net.minecraft.world.chunk.storage.AnvilChunkLoader)) {
				if (!loadingChunks.add(pos))
					net.minecraftforge.fml.common.FMLLog.bigWarning(
							...
							x, z, this.world.provider.getDimension());
				if (chunk == null)
			...
			if (!this.droppedChunksSet.isEmpty()) {
				...
				for (int i = 0; i < 100 && iterator.hasNext(); iterator.remove()) {
					...
					if (chunk != null && chunk.unloadQueued) {
						chunk.onUnload();
						net.minecraftforge.common.ForgeChunkManager.putDormantChunk(ChunkPos.asLong(chunk.x, chunk.z),
								chunk);
						this.saveChunkData(chunk);
						this.saveChunkExtraData(chunk);
						...
						if (id2ChunkMap.size() == 0 && net.minecraftforge.common.ForgeChunkManager
								.getPersistentChunksFor(this.world).size() == 0
